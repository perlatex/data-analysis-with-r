...
- 以血液存储时间分组，统计复发的占比，最后画出柱状图，并在图中标记比例
```{r}
df %>% 
  group_by(rbc_age_group_new) %>% 
  summarise(
  ...
  ggplot(aes(x = rbc_age_group_new, y = prop_recurrence)) +
  geom_col(width = 0.5) +
  geom_text(aes(label = scales::label_percent(accuracy = 0.02)(prop_recurrence)), vjust = -0.5)
```
