...
```{r}
mod2 <- lm(earn ~ height + edu, data = wages)
```
...
race变量就是数据框wages的一个分类变量，代表四个不同的种族。用分类变量做回归，本质上是各组之间的进行比较。我们看到输出结果，只有`race_hispanic`、`race_other`和`race_white`三个系数和`Intercept`截距，`race_black`去哪里了呢？
...
事实上，分类变量在R语言代码里，会转换成0和1这种**虚拟变量**，然后再计算。比如

$$
Y_{i}=\beta_{0}+\beta_{1}\textrm{height}_{i}+\beta_{2}\textrm{sex}_{i}+\epsilon_{i}
$$

构建一个新变量gender，将sex中`(male, female)`替换成`(1, 0)`
...
然后放入`lm()`
```{r}
mod5a <- lm(earn ~ height + gender, data = wages_gender)
coef(mod5a)
```
...
```{r}
mod6 <- lm(earn ~ height + gender + height:gender, data = wages_gender)
```
...
```{r}
mod7 <- lm(earn ~ height + height:gender, data = wages_gender)
coef(mod7)
```
...
同样假定男性(gender = 1)和女性(gender = 0)，那么
$$
\begin{aligned}
...
- 注意到，mod6和mod7是两个不同的模型, 
  - mod7中男女拟合直线在y轴的截距是相同的，而mod6在y轴的截距是不同的
