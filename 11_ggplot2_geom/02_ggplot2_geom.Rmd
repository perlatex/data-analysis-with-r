...
install.packages( 
 c("ggridges", "see", "palmerpenguins", "datapasta", "colorspace", 
   ...
   "readxl",  "showtext", "styler", "remotes", "rnaturalearth") 
)
```
...
> 思路：前面提到aes(fill = species)是3种species对应3种颜色，那么我们能否构建一个新变量is_Gentoo，它只包含(是Gentoo、不是Gentoo)，正好对应两种颜色（高亮色、灰色）
...
**注意这张图的继承关系**，包括`data =` 以及 `aes()` 的继承，具体说：
第一个图层用的数据是什么，映射关系是什么？
第二个图层用的数据是什么，映射关系是什么？
