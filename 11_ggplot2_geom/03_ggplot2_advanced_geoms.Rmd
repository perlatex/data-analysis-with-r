...
```{r}
penguins %>% 
  ggplot(aes(y = species, fill = island)) +
...
penguins %>%
  ...
  geom_smooth(
    method = "gam", formula = y ~ s(x, bs = "cs", fx = TRUE, k = 20), 
    se = FALSE, 
    aes( color = "orange")
  ) +
  scale_color_identity(
    name = "model fit",
    breaks = c("black", "red", "blue", "orange"),
    labels = c("linear", "quadratic", "cubic", "gam"),
    guide = "legend"
    )
```
