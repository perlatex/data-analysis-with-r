...
录取是否与学生GRE考试成绩有关系？我们能用GRE成绩**预测**录取情况吗？
...
- `logit()`是连接函数（还记得前面那个伟大的函数？伟大的函数就是这个连接函数的反函数）
$$
\begin{align*}
...
有时也称$\text{logit}(p_{i})$为对数比率(log-odds).
...
我们把`logit`移到等式右边，等式左边由**对数比率**变成**概率**，更符合人的直觉。上面公式可以写成
...
下面，我们用三种方式计算原数据框中每个`gre`值对应的录取概率
...
有时候，我们需要对给定的GRE分数，用建立的模型**预测**被录取的概率。这里也有几个方法
...
2. 用`broom`包，更顺滑（注意，预测时用的参数是`newdata =` ）
```{r}
model_logit %>%
  broom::augment(
...
模型 `model_logit2` 是`gre`, `gpa`和`rank`三个解释变量， 因此，在模型预测的时候，提供的新数据框也应该包含 `gre`, `gpa`和`rank`三列。
简单起见，`gre`值为所有同学的平均`gre`值；`gpa`值为所有同学的平均`gpa`值；`rank`为学校声望的四个等级。
...
由此可以画出，来自不同声望等级院校的学生，研究生录取概率随GRE成绩的变化。这里为了画出置信区间，我们先估算线性组合的拟合值(即logit scale)，连同标准误计算置信区间后，再转回到概率。
...
model_logit2 %>%
  ...
  mutate(
    ...
    LL = inv_logit(.fitted - (1.96 * .se.fit)),
    UL = inv_logit(.fitted + (1.96 * .se.fit))
  ) %>%
  ggplot(aes(x = gre, y = prob)) +
  geom_ribbon(
    aes(ymin = LL, ymax = UL, fill = rank),
    alpha = 0.3
  ) +
